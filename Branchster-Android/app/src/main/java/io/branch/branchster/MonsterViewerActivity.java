...
public class MonsterViewerActivity extends FragmentActivity implements InfoFragment.OnFragmentInteractionListener {
    ...
    private void initUI() {
        ...
        if (myMonsterObject != null) {
            ...
            /*
            * 2c). Using an asynchronous method inside `initUI()`, get a shortURL with params,
            ...
            * `monsterUrl` TextView, and be sure to move
            * the`progressBar.setVisibility(View.GONE);` inside the completionhandler
            * */

            LinkProperties lp = new LinkProperties();
            lp.setChannel("sms");
            lp.setFeature("shareUrl");
            lp.setChannel("SMS1");

            BranchUniversalObject branchUniversalObject = myMonsterObject.convertToBranchObj();
            branchUniversalObject.generateShortUrl(this, lp, new Branch.BranchLinkCreateListener() {
                @Override
                public void onLinkCreate(String url, BranchError error) {
                    if(error == null) {
                        monsterUrl.setText(url);
                        Log.i("BRANCH SDK", "share branch link: " + url);
                    }
    ...
    //Log share event via Branch
    private void logShareEvent(){
        BranchEvent branchEvent = new BranchEvent(BRANCH_STANDARD_EVENT.SHARE);
        branchEvent.addContentItems(myMonsterObject.convertToBranchObj());
        branchEvent.logEvent(MonsterViewerActivity.this);
    }
    ...
    private void shareMyMonster() {
        ...
        /*
         *  2e). Only attempt to send the SMS if there is no error in getting the Branch link.
         * */
        ...
        /*
         *  2d). Inside `shareMyMonster`, get a short URL with params. Get the params
    ...
    protected void onStart() {
        super.onStart();
        //Call view event for question 2b
        logViewEvent();
    }

    /*2b) Track that the user visited the monster view page (call event "monster_view") on the MonsterViewerActivity.
    * This time, include state information (specified in `myMonsterObject.monsterMetaData()`)
    * */
    private void logViewEvent(){
        BranchEvent branchEvent = new BranchEvent("monster_view");
        branchEvent.addContentItems(myMonsterObject.convertToBranchObj()).logEvent(MonsterViewerActivity.this);
    }
